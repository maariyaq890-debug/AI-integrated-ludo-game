...
class SplashScreen:
    def __init__(self, root, on_complete_callback):
        ...
        # Create canvas
        self.canvas = Canvas(
            self.window, bg="black", width=800, height=630, highlightthickness=0
        )
    ...
    def animate_text(self):
        ...
        if items:
            self.canvas.itemconfig(
                items[0],
                fill=f"#{int(intensity * 0.4):02x}{int(intensity * 0.4):02x}{int(intensity * 0.4):02x}",
            )
        ...
        if ret:
            try:
                ...
                self.canvas.create_image(
                    400, 315, image=self.current_frame_photo, tags="video_frame"
                )
    ...
    def smooth_fade_out(self):
        ...
        for i in range(num_layers):
            ...
            rect = self.canvas.create_rectangle(
                0, 0, 800, 630, fill=fade_color, outline="", tags="fade_overlay"
            )
        ...
        if self.fade_alpha > 0.7:
            ...
            final_color = (
                f"#{final_darkness:02x}{final_darkness:02x}{final_darkness:02x}"
            )
            final_rect = self.canvas.create_rectangle(
                0, 0, 800, 630, fill=final_color, outline="", tags="fade_overlay"
            )
...
class GameModeSelector:
    def __init__(self, root, on_mode_selected, parent_window):
        ...
        # Create canvas
        self.canvas = Canvas(
            self.window, bg="#1a1a2e", width=650, height=500, highlightthickness=0
        )
        self.canvas.pack(fill=BOTH, expand=1)
    ...
    def create_mode_selection(self):
        ...
        modes = [
            (
                ...
                "#2ecc71",
                "classic",
                "Traditional Ludo with standard rules",
            ),
            ...
            (
                ...
                "#9b59b6",
                "team_up",
                "Classic mode with team partnerships",
            ),
        ]
        ...
        for i, (text, color, mode_id, description) in enumerate(modes):
            ...
            desc_text = self.canvas.create_text(
                ...
                fill="#f0f0f0",
                tags=f"{mode_id}_desc",
            )

            # Group all button elements
            button_tags = [
                f"{mode_id}_container",
                f"{mode_id}_bg",
                f"{mode_id}_shadow",
                f"{mode_id}_title",
                f"{mode_id}_desc",
            ...
            for tag in button_tags:
                self.canvas.tag_bind(
                    tag, "<Button-1>", lambda e, m=mode_id: self.select_mode(m)
                )
                self.canvas.tag_bind(
                    tag, "<Enter>", lambda e, m=mode_id: self.button_hover(m)
                )
                self.canvas.tag_bind(
                    tag, "<Leave>", lambda e, m=mode_id: self.button_leave(m)
                )

        # Back button with better styling
        back_y = 420
        back_bg = self.canvas.create_rectangle(
            230,
            back_y - 20,
            420,
            back_y + 20,
        ...
        back_shadow = self.canvas.create_rectangle(
            233,
            back_y - 17,
            423,
            back_y + 23,
    ...
    def button_hover(self, button_id):
        if button_id == "classic":
            ...
            self.canvas.coords(
                "classic_bg", coords[0] - 2, coords[1] - 2, coords[2] + 2, coords[3] + 2
            )
            ...
            self.canvas.coords(
                "rush_bg", coords[0] - 2, coords[1] - 2, coords[2] + 2, coords[3] + 2
            )
            ...
            self.canvas.coords(
                "team_up_bg", coords[0] - 2, coords[1] - 2, coords[2] + 2, coords[3] + 2
            )
            ...
            self.canvas.coords(
                "back_bg", coords[0] - 2, coords[1] - 2, coords[2] + 2, coords[3] + 2
            )
    ...
    def animate_selection(self, mode):
        ...
        self.window.after(
            100, lambda: self.canvas.itemconfig(f"{mode}_bg", fill=original_color)
        )
        self.window.after(
            200, lambda: self.canvas.itemconfig(f"{mode}_bg", fill=flash_color)
        )
        self.window.after(
            300, lambda: self.canvas.itemconfig(f"{mode}_bg", fill=original_color)
        )
...
class LudoFrontPage:
    def __init__(self, root):
        ...
        # Create main canvas
        self.canvas = Canvas(
            self.window, bg="#1a1a2e", width=800, height=630, highlightthickness=0
        )
        self.canvas.pack(fill=BOTH, expand=1)

        # Bind global click sound for all Tk Buttons in this window
        try:
            self.window.bind_class(
                "Button", "<Button-1>", lambda e: self._play_click_sound(), add="+"
            )
        except Exception:
            pass
    ...
    def create_background(self):
        ...
        for i in range(50):
            ...
            self.canvas.create_rectangle(
                0, i * 13, 800, (i + 1) * 13, fill=color, outline=color
            )
    ...
    def create_dice(self):
        ...
        for i, (x, y) in enumerate(self.dice_positions):
            dice_bg = self.canvas.create_rectangle(
                x - 25,
                y - 25,
                x + 25,
                y + 25,
    ...
    def create_buttons(self):
        # Play button
        play_btn_bg = self.canvas.create_rectangle(
            300,
            360,
            500,
            400,
        ...
        # Game Modes button
        modes_btn_bg = self.canvas.create_rectangle(
            300,
            420,
            500,
            460,
        ...
        # Instructions button
        inst_btn_bg = self.canvas.create_rectangle(
            300,
            480,
            500,
            520,
        ...
        # Exit button
        exit_btn_bg = self.canvas.create_rectangle(
            300,
            540,
            500,
            580,
        ...
        self.canvas.tag_bind(
            "modes_btn", "<Enter>", lambda e: self.button_hover("modes")
        )
        self.canvas.tag_bind(
            "modes_btn", "<Leave>", lambda e: self.button_leave("modes")
        )
        ...
        for i in range(4):
            ...
            if items:
                ...
                if len(coords) == 4:
                    ...
                    self.canvas.coords(
                        items[0], base_x - 15, new_y - 15, base_x + 15, new_y + 15
                    )
    ...
    def open_mode_selection(self):
        ...
        GameModeSelector(
            mode_window, self.on_mode_selected, self.window
        )  # Pass parent window
        ...
        if items:
            for item in items:
                if self.canvas.type(item) == "rectangle":
                    ...
                    if button_type == "play":
                        self.window.after(
                            150, lambda: self.canvas.itemconfig(item, fill="#27ae60")
                        )
                    elif button_type == "modes":
                        self.window.after(
                            150, lambda: self.canvas.itemconfig(item, fill="#8e44ad")
                        )
    ...
    def start_classic_game(self):
        ...
        try:
            ...
            try:
                ...
                block_six_side = ImageTk.PhotoImage(
                    Image.open("Images/6_block.png").resize((33, 33), Image.LANCZOS)
                )
                block_five_side = ImageTk.PhotoImage(
                    Image.open("Images/5_block.png").resize((33, 33), Image.LANCZOS)
                )
                block_four_side = ImageTk.PhotoImage(
                    Image.open("Images/4_block.png").resize((33, 33), Image.LANCZOS)
                )
                block_three_side = ImageTk.PhotoImage(
                    Image.open("Images/3_block.png").resize((33, 33), Image.LANCZOS)
                )
                block_two_side = ImageTk.PhotoImage(
                    Image.open("Images/2_block.png").resize((33, 33), Image.LANCZOS)
                )
                block_one_side = ImageTk.PhotoImage(
                    Image.open("Images/1_block.png").resize((33, 33), Image.LANCZOS)
                )
            except AttributeError:
                # Fallback for older PIL versions
                block_six_side = ImageTk.PhotoImage(
                    Image.open("Images/6_block.png").resize((33, 33), Image.ANTIALIAS)
                )
                block_five_side = ImageTk.PhotoImage(
                    Image.open("Images/5_block.png").resize((33, 33), Image.ANTIALIAS)
                )
                block_four_side = ImageTk.PhotoImage(
                    Image.open("Images/4_block.png").resize((33, 33), Image.ANTIALIAS)
                )
                block_three_side = ImageTk.PhotoImage(
                    Image.open("Images/3_block.png").resize((33, 33), Image.ANTIALIAS)
                )
                block_two_side = ImageTk.PhotoImage(
                    Image.open("Images/2_block.png").resize((33, 33), Image.ANTIALIAS)
                )
                block_one_side = ImageTk.PhotoImage(
                    Image.open("Images/1_block.png").resize((33, 33), Image.ANTIALIAS)
                )
            ...
            messagebox.showerror(
                "Error",
                "Could not start Classic Mode. Please check if Ludo_game_with_Sam.py exists.",
            )
    ...
    def start_rush_game(self):
        ...
        try:
            ...
            traceback.print_exc()
            messagebox.showerror(
                "Error", f"Could not start Rush Mode.\nError: {str(e)}"
            )
    ...
    def start_team_up_game(self):
        ...
        try:
            ...
            traceback.print_exc()
            messagebox.showerror(
                "Error", f"Could not start Team-Up Mode.\nError: {str(e)}"
            )
        ...
        def _play():
            try:
                ...
                try:
                    ...
                    winsound.PlaySound(
                        "sounds/Jump.wav", winsound.SND_FILENAME | winsound.SND_ASYNC
                    )
                    return
                except Exception:
                    pass
